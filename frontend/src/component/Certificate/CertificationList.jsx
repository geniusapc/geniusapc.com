...
export default function CertificateNCertifications (){
    ...
    useEffect(()=>{hash==="#certifications" && certifications.current.scrollIntoView()}, [hash])
    ...
    return(
        <div className="container certifications" ref={certifications} >
                <section id="certificate">
                    <h1>Certificates</h1>
                    <ul>
                        {certificates.major.map((e)=><li key={e.id}> <img src={e.icon} alt="certificate" /> <span>{e.name}{openExternalLink(e.link)}</span></li>)} 
                    </ul>  
                </section> 
